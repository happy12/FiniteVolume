...
// See RANSFVMSolver.h for the input/output contract.
RANSFVMSolver::RANSFVMSolver(const UnstructuredMesh& input_mesh, const std::vector<RANSBoundaryCondition>& boundary_conditions,
...
// See RANSFVMSolver.h for the input/output contract.
void RANSFVMSolver::build_boundary_fields(const std::vector<double>& u, const std::vector<double>& v,
...
// See RANSFVMSolver.h for the input/output contract.
void RANSFVMSolver::build_boundary_nut(const std::vector<double>& nut_field, std::vector<double>& boundary_nut) const
{
    ...
    for (size_t i = 0; i < mesh.faces.size(); ++i) {
        const Face& face = mesh.faces[i];
        if (face.cell_right != -1) continue;
        const RANSBoundaryCondition& bc = bcs.at(face.patch_id);
...
// See RANSFVMSolver.h for the input/output contract and methodology.
void RANSFVMSolver::step()
{
...
// See RANSFVMSolver.h for the input/output contract.
void RANSFVMSolver::run(int total_steps)
{
...
// See RANSFVMSolver.h for the input/output contract.
double RANSFVMSolver::compute_dt() const
{
...
// See RANSFVMSolver.h for the input/output contract.
std::vector<WallFaceSample> RANSFVMSolver::compute_wall_traction_samples(const std::vector<int>& wall_faces) const
{
    const size_t N = mesh.cells.size();
...
// See RANSFVMSolver.h for the input/output contract.
std::vector<BoundaryLayerProfile> RANSFVMSolver::compute_boundary_layer_profile_samples(
...
// See RANSFVMSolver.h for the input/output contract.
std::vector<BoundaryLayerProfile> RANSFVMSolver::compute_boundary_layer_profile_samples_point_location(
...
// See RANSFVMSolver.h for the input/output contract.
EulerState RANSFVMSolver::ghost_state(const Face& face, const EulerState& U_L) const
{
    const NSBoundaryCondition& bc = bcs.at(face.patch_id).ns;
