...
#ifndef RANSTURBULENCESASOLVER_H_INCLUDED
#define RANSTURBULENCESASOLVER_H_INCLUDED
...
// (see build_boundary_nut() in RANSTurbulenceSASolver.cpp), and Outflow's is a
// zero-order extrapolation, matching every other field's Outflow treatment
// in this codebase.
struct RANSBoundaryConditionSA {
...
// has_viscous_term in RANSTurbulenceSASolver.cpp.
class RANSTurbulenceSASolver {
public:
    // Input:
    ...
    //                          this choice; see RANSBoundaryConditionSA::farfield_nut)
    ...
    RANSTurbulenceSASolver(const UnstructuredMesh& input_mesh, const std::vector<RANSBoundaryConditionSA>& boundary_conditions,
    ...
    // Mean-flow-only overload: overwrites U but leaves nut exactly as this
    // solver's constructor already initialized it (uniform initial_nut). Used
    // when resuming from a checkpoint written by a different equation set/
    // turbulence model (see Checkpoint::read_flow_field() and main.cpp's
    // run_ransSA()) -- the flow field carries over, but nut restarts fresh
    // rather than from a value that was never actually transported by SA.
...
private:
    ...
    std::vector<RANSBoundaryConditionSA> bcs;
...
#endif // RANSTURBULENCESASOLVER_H_INCLUDED
